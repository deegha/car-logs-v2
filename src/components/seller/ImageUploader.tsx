...
export function ImageUploader({
  ...
  useEffect(() => {
    const uploaded = images.filter((img) => img.status === "done" && img.cloudUrl).map((img) => img.cloudUrl!)
  ...
  async function startUpload(id: string, file: File) {
    ...
    try {
      ...
      setImages((prev) =>
        prev.map((img) => (img.id === id ? { ...img, status: "error", error: "Network error" } : img)),
      )
    }
  }
      ...
      >
        {images.map((img, i) => (
          ...
          >
            ...
            {/* Cover badge */}
            {i === 0 && img.status === "done" && (
              <span className="absolute left-1 top-1 rounded-sm bg-primary-600 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-white">
                Cover
              </span>
            )}
            ...
            {/* Done indicator */}
            {img.status === "done" && (
              <div className="absolute right-1 top-1 flex h-4 w-4 items-center justify-center rounded-full bg-green-500/90">
            ...
            <button
              ...
              className={cn(
                "absolute bottom-0 right-0 flex h-7 w-7 items-center justify-center rounded-tl-lg bg-black/60 text-white transition-colors hover:bg-red-600/80 active:bg-red-700",
        ...
        {remaining > 0 && (
          <button
            type="button"
            onClick={openPicker}
            className="aspect-square rounded-lg border-2 border-dashed border-border bg-background-subtle flex flex-col items-center justify-center gap-1.5 text-foreground-muted transition-colors hover:border-primary-400 hover:bg-primary-50 hover:text-primary-600 active:bg-primary-100"
