        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            {status === "PENDING" && (
              <>
                <form action={approveWithId}>
                  <button
                    type="submit"
                    className="rounded-md bg-emerald-600 px-3 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700 transition-colors"
                  >
                    Approve
                  </button>
                </form>
                <form action={rejectWithId}>
                  <button
                    type="submit"
                    className="rounded-md bg-red-600 px-3 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-red-700 transition-colors"
                  >
                    Reject
                  </button>
                </form>
            ...
            <Link
              ...
              className="rounded-md border border-border bg-background px-3 py-1.5 text-sm font-medium text-foreground hover:bg-background-subtle transition-colors"
            >
              Edit
            </Link>
            <Link
          ...
          <div className="mb-6 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3.5">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-amber-700">
                This listing is not yet visible to buyers. Review the details below and approve or reject.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
          ...
          <div className="flex flex-col gap-6 lg:col-span-3">
            ...
            <div className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
              <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
                <h2 className="text-sm font-semibold text-foreground">
                  Photos
                  <span className="ml-1.5 text-xs font-normal text-foreground-muted">({photos.length})</span>
                </h2>
                {photos.length === 0 && (
                  ...
                  {coverPhoto && (
                    <div className="relative aspect-[16/9] w-full overflow-hidden rounded-xl bg-background-subtle">
                      <Image
                        ...
                        priority
                      />
                      <span className="absolute left-2.5 top-2.5 rounded-md bg-black/60 px-2 py-0.5 text-xs font-semibold text-white backdrop-blur-sm">
                        Cover
                      </span>
                    </div>
                  )}
                  ...
                  {otherPhotos.length > 0 && (
                    <div className="grid grid-cols-4 gap-2">
                      {otherPhotos.map((img, i) => (
                        <div key={img.id} className="relative aspect-video overflow-hidden rounded-lg bg-background-subtle">
                          <Image
                            src={img.url}
            ...
            {car.emissionTestUrl && (
              <div className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
                <div className="border-b border-border px-5 py-3.5">
                  <h2 className="text-sm font-semibold text-foreground">Emission Test Certificate</h2>
                </div>
                <div className="p-3">
                  <a href={car.emissionTestUrl} target="_blank" rel="noopener noreferrer" className="group relative block overflow-hidden rounded-xl">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-black/30 opacity-0 transition-opacity group-hover:opacity-100">
                      <span className="rounded-md bg-white/90 px-3 py-1.5 text-xs font-semibold text-foreground">Open full size ↗</span>
                    </div>
                  </a>
                </div>
              </div>
          ...
          <div className="flex flex-col gap-6 lg:col-span-2">
            ...
            <div className="rounded-2xl border border-border bg-background p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-2xl font-bold text-primary-600">{formatPrice(Number(car.price))}</p>
                  {car.isNegotiable && (
                ...
                <span className={`shrink-0 rounded-full border px-2.5 py-0.5 text-xs font-semibold ${conditionColors[car.condition] ?? "bg-background-subtle text-foreground border-border"}`}>
            ...
            <div className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
              ...
              <div className="divide-y divide-border">
                {[
                  ...
                  .filter(Boolean)
                  .map((row) => (
                    <div key={(row as { label: string }).label} className="flex items-center gap-3 px-5 py-2.5">
              ...
              <div className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
                ...
                <div className="px-5 py-4">
                    ...
                    <div className="mt-4 flex flex-col gap-2">
                      {primaryPhone && (
                        <a
                          ...
                          className="flex items-center gap-2.5 rounded-lg border border-border bg-background-subtle px-3.5 py-2.5 text-sm font-medium text-foreground hover:bg-background transition-colors"
                        >
                      ...
                      {whatsappPhone && (
                        <a
                          ...
                          className="flex items-center gap-2.5 rounded-lg border border-green-200 bg-green-50 px-3.5 py-2.5 text-sm font-medium text-green-800 hover:bg-green-100 transition-colors"
                        >
                  ...
                  <div className="mt-4 flex flex-wrap gap-3 border-t border-border pt-4 text-xs text-foreground-muted">
                    <span>
                      Status:{" "}
                      <span className={`font-semibold ${car.seller.status === "ACTIVE" ? "text-emerald-600" : "text-red-600"}`}>
                        {car.seller.status}
                      </span>
                    </span>
                    <span>
                      Member since{" "}
                      {new Date(car.seller.createdAt).toLocaleDateString("en-AU", { month: "short", year: "numeric" })}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-border bg-background-subtle px-5 py-4 text-xs text-foreground-muted shadow-sm">
              <p>Listing ID: <span className="font-mono font-semibold text-foreground">#{car.id}</span></p>
              {car.slug && <p className="mt-1">Slug: <span className="font-mono text-foreground">{car.slug}</span></p>}
            </div>
          </div>
        </div>
      </div>
